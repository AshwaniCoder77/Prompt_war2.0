...
// Static base English dictionary for the app
const enDict = {
  // Sidebar
  "nav.home": "Home",
  ...
  "resources.visitChannel": "Official YouTube Channel (ECI)",
  "resources.video1": "Mera Vote Meri Duty",
  "resources.video2": "Stamp your mark with your vote",
...
export const LanguageProvider = ({ children }) => {
  ...
  const [dict, setDict] = useState(enDict);

  useEffect(() => {
    localStorage.setItem('vote_lang', lang);

    // Get the locale key (e.g., from './locales/hi.json' -> 'hi')
    const localePath = `./locales/${lang}.json`;
    if (locales[localePath]) {
      setDict(locales[localePath]);
    } else {
